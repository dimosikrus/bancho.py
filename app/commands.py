...
async def request(ctx: Context) -> Optional[str]:
    ...
    if bmap.status != RankedStatus.Pending and bmap.status != RankedStatus.Qualified and bmap.status != RankedStatus.Loved:
        return "Only pending or qualified maps may be requested for status change."
...
str_priv_dict = {
    ...
    "communitymanager": Privileges.COMMUNITYMANAGER,
