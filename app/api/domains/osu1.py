...
async def get_leaderboard_scores(
    ...
    query = [
        ...
        "s.max_combo, s.n50, s.n100, s.n300, s.pp, "
    ...
    if leaderboard_type == LeaderboardType.Mods:
        ...
        params["mods"] = mods
        query.append("AND s.status IN (1, 2)")
...
async def getScores(
    ...
    if personal_best_score_row is not None:
        ppy = round(personal_best_score_row['pp'])
        response_lines.append(
            SCORE_LISTING_FMTSTR.format(
                **personal_best_score_row,
                name=f'{player.full_name} {ppy}pp',
                userid=player.id,
...
async def osuSeasonal():
    bgs = [
    "https://osu.turuturuone.xyz/static/images/seasonal/1.png",
    "https://osu.turuturuone.xyz/static/images/seasonal/2.jpg",
    "https://osu.turuturuone.xyz/static/images/seasonal/3.png",
    "https://osu.turuturuone.xyz/static/images/seasonal/4.jpg",
    "https://osu.turuturuone.xyz/static/images/seasonal/5.jpg",
    "https://osu.turuturuone.xyz/static/images/seasonal/6.jpg",
    "https://osu.turuturuone.xyz/static/images/seasonal/7.jpg",
    "https://osu.turuturuone.xyz/static/images/seasonal/8.jpg",
    "https://osu.turuturuone.xyz/static/images/seasonal/9.jpg",
    "https://osu.turuturuone.xyz/static/images/seasonal/10.jpg",
    "https://osu.turuturuone.xyz/static/images/seasonal/11.jpg",
    "https://osu.turuturuone.xyz/static/images/seasonal/12.jpg",
    "https://osu.turuturuone.xyz/static/images/seasonal/13.png",
    "https://osu.turuturuone.xyz/static/images/seasonal/14.png"
    ]
    return ORJSONResponse(bgs)
...
@router.get("/web/postback.php")
async def top_g(
    p_resp: str = Query(...),
    ip: str = Query(...)
):
    wb = DiscordWebhook(url="https://discord.com/api/webhooks/1112385741373321237/ciFsjvHWJlYFgLfH10-IXDICUY90HBQnSvUdcUZiuQYuFxh_KEeMRM1DSh1fXewzu_4u")
    if (ip_valid := validate_ip(ip)):
        ds_embed = DiscordEmbed(
            title="TopG Voted!",
            description=f"{p_resp} voted for Turu\nip: `{ip}`\nip_validation: {ip_valid}",
            color="03b2f8"
        )
    else:
        ds_embed = DiscordEmbed(
            title="TopG ip validation Error",
            description=f"{p_resp} voted, but\nip: `{ip}`\nip_validation: {ip_valid}",
            color="882211"
        )
...
@router.get("/web/osuserverlist")
async def osuserverlist(
    username: str = Query(...),
    key: str = Query(...)
):  
    valid_key = "bLwkgGCVWNJT0xh21H5yzy38k"
    wb = DiscordWebhook(url="https://discord.com/api/webhooks/1112385741373321237/ciFsjvHWJlYFgLfH10-IXDICUY90HBQnSvUdcUZiuQYuFxh_KEeMRM1DSh1fXewzu_4u")
    if key == valid_key:
        ds_embed = DiscordEmbed(
            title="osuserverlist Voted!",
            description=f"{username} voted for Turu!\nKey validation success!",
            color="03b2f8"
        )
        if not(t := await app.state.sessions.players.from_cache_or_sql(name=username)):
            pass
        if not(timespan := timeparse("3d")):
            pass
        if t.donor_end == None:
            t.donor_end = 0 #bug fix NoneType
        ...
        ds_embed = DiscordEmbed(
            title="osuserverlist Wrong Voted!",
            description=f"{username} give not valid key!\nKey validation not success!",
            color="882211"
        )
    wb.add_embed(ds_embed)
    wb.execute()
